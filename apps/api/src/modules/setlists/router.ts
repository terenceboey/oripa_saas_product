...
function setlistScopeWhere(game: string): Prisma.CatalogSetWhereInput {
  if (game === "ALL") return {};
...
setlistRouter.get("/v1/public/setlists/stats", async (req, res) => {
  try {
    ...
    // Match the public tab scopes instead of grouping only by raw game code.
    const total = await prisma.catalogSet.count({ where: baseWhere });
    const pokemon = await prisma.catalogSet.count({ where: { ...baseWhere, ...setlistScopeWhere("POKEMON") } });
    const onePiece = await prisma.catalogSet.count({ where: { ...baseWhere, ...setlistScopeWhere("ONE_PIECE") } });
    const pokemonJapan = await prisma.catalogSet.count({ where: { ...baseWhere, ...setlistScopeWhere("POKEMON_JAPAN") } });
